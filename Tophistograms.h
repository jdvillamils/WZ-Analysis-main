...
void TopAnalysis::define_histograms()
{
  ...
  hist_leadleptch2   = new TH1F("hist_leadleptch2",  "Leading Lepton Charge; Q^{leadlep}; Leptons", 7, -1.75, 1.75);
  hist_leadleptID2   = new TH1F("hist_leadleptID2",  "Leading Lepton Absolute PDG ID; |PDG ID|^{leadlep}; Leptons",  31, -0.5, 30.5);
  hist_leadlept_ptc2  = new TH1F("hist_leadlept_ptc2", "Leading Lepton Relative Transverse Momentum Isolation; ptconerel30^{leadlep}; Leptons", 40, -0.1, 0.4);
  hist_leadleptetc2  = new TH1F("hist_leadleptetc2", "Leading Lepton Relative Transverse Energy Isolation; etconerel20^{leadlep}; Leptons", 40, -0.1, 0.4);
  hist_leadlepz02    = new TH1F("hist_leadlepz02",   "Leading Lepton z0 impact parameter; z_{0}^{leadlep} [mm]; Leptons", 40, -1, 1);
  hist_leadlepd02    = new TH1F("hist_leadlepd02",   "Leading Lepton d0 impact parameter; d_{0}^{leadlep} [mm]; Leptons", 40, -1, 1);
  // CutFlow
  hist_cutflow = new TH1F("hist_cutflow","Cutflow; Cut; Events",10,0,10);
  // Top, W, ZPrime mass histograms
  hist_Zmass = new TH1F("hist_Zmass","Z Invariant Mass; M_{Z};Events", 40, 0, 200);
  ...
  hist_Zmasss = new TH1F("hist_Zmasss","Z Invariant Mass No Cut; M_{Z};Events", 40, 0, 200);
  hist_Wmasss = new TH1F("hist_Wmasss","Z Invariant Mass No Cut; M_{Z};Events", 40, 0, 200);
...
void TopAnalysis::FillOutputList()
{
  ...
  GetOutputList()->Add(hist_leadleptch2);
  GetOutputList()->Add(hist_leadleptID2);
  GetOutputList()->Add(hist_leadlept_ptc2);
  GetOutputList()->Add(hist_leadleptetc2);
  GetOutputList()->Add(hist_leadlepz02);
  GetOutputList()->Add(hist_leadlepd02);
  ...
  // Add Top and W+ mass histograms
  GetOutputList()->Add(hist_Zmass);
  GetOutputList()->Add(hist_Wmass);
  GetOutputList()->Add(hist_mazz);
  GetOutputList()->Add(hist_cutflow);
  GetOutputList()->Add(hist_Zmasss);
  GetOutputList()->Add(hist_Wmasss);
}
...
void TopAnalysis::WriteHistograms()
{
  ...
  hist_leadleptch2->Write();
  hist_leadleptID2->Write();
  hist_leadlept_ptc2->Write();
  hist_leadleptetc2->Write();
  hist_leadlepz02->Write();
  hist_leadlepd02->Write();
  ...
  // Write Top and W+ mass histograms
  hist_Zmass->Write();
  hist_Wmass->Write();
  hist_mazz->Write();
  hist_cutflow->Write();
...
void TopAnalysis::FillHistogramsGlobal( double m, float w , TString s)
{
  ...
  if (s.Contains("hist_cutflow")) hist_cutflow->Fill(m,w);
    
  if (s.Contains("hist_Zmass")) hist_Zmass->Fill(m,w);
    
  if (s.Contains("hist_Wmass")) hist_Wmass->Fill(m,w);

  if (s.Contains("hist_Zmasss")) hist_Zmasss->Fill(m,w);
    
  if (s.Contains("hist_Wmasss")) hist_Wmasss->Fill(m,w);
