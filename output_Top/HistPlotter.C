...
void HistPlotter(){
  ...
  std::ostringstream Entries_db;
  Entries_db << setprecision (2) << h_db->Integral(0,h_db->GetNbinsX()+1);
  std::string sEntries_db = Entries_db.str();
  cout<<sEntries_db<<"    "<<endl;
   
  std::ostringstream Entries_drellyan;
  Entries_drellyan << setprecision (2) << h_drellyan->Integral(0,h_drellyan->GetNbinsX()+1);
  std::string sEntries_drellyan = Entries_drellyan.str();
  cout<<sEntries_drellyan<<"    "<<endl;
   
  std::ostringstream Entries_wjets;
  Entries_wjets << setprecision (4) << h_wjets->Integral(0,h_wjets->GetNbinsX()+1);
  std::string sEntries_wjets = Entries_wjets.str();
  cout<<sEntries_wjets<<"    "<<endl;
   
  std::ostringstream Entries_zjets;
  Entries_zjets << setprecision (3) << h_zjets->Integral(0,h_zjets->GetNbinsX()+1);
  std::string sEntries_zjets = Entries_zjets.str();
  cout<<sEntries_zjets<<"    "<<endl;
   
  std::ostringstream Entries_singletop;
  Entries_singletop << setprecision (4) << h_singletop->Integral(0,h_singletop->GetNbinsX()+1);
  std::string sEntries_singletop = Entries_singletop.str();
  cout<<sEntries_singletop<<"    "<<endl;
  ...
  TH1F* h_signal = (TH1F*) f_sig->Get(histname.c_str());
    // se comenta para TopMass y WMass
  ...
  std::ostringstream Entries_signal;
  Entries_signal << setprecision (8) << h_signal->Integral(0,h_signal->GetNbinsX()+1);
  std::string sEntries_signal = Entries_signal.str();
  cout<<"Signal:"<<sEntries_signal<<endl;
 
  std::ostringstream Entries_bkg;
  Entries_bkg << setprecision (5) << h_ttbar->Integral(0,h_ttbar->GetNbinsX()+1) + h_db->Integral(0,h_db->GetNbinsX()+1) + h_singletop->Integral(0,h_singletop->GetNbinsX()+1) + h_wjets->Integral(0,h_wjets->GetNbinsX()+1) + h_zjets->Integral(0,h_zjets->GetNbinsX()+1) + h_drellyan->Integral(0,h_drellyan->GetNbinsX()+1);
  ...
  //h_stack->SetMaximum(2*h_stack->GetMaximum());//(20*h_stack->GetMaximum());
  h_stack->SetMaximum(80); //Set Maximum in Y axis
  ...
  else if(histname.find("hist_Zmass") != std::string::npos)h_stack->GetXaxis()->SetRangeUser(0, 200);
  ...
  leg->AddEntry(h_singletop,("Single top:"+sEntries_singletop).c_str(),"f");
  leg->AddEntry(h_zjets,("Z:"+sEntries_zjets).c_str(),"f");
  leg->AddEntry(h_wjets,("W:"+sEntries_wjets).c_str(),"f");
  leg->AddEntry(h_drellyan,("Drell-Yan:"+sEntries_drellyan).c_str(),"f");
  leg->AddEntry(h_db,("Diboson:"+sEntries_db).c_str(),"f");
  leg->AddEntry(h_err,"MC stat. uncertainty","f");
    // Se comenta la siguiene linea para TOpMass y Wmasss
  //leg->AddEntry(h_signal,("Z'(" + signalmass + " GeV)#rightarrow t#bar{t} x 10^{4}:"+sEntries_signal).c_str(),"l");
  leg->Draw();
